...
// Select all buttons with class 'hide-row'
document.querySelectorAll('.hide-row').forEach(button => {
    button.addEventListener('click', function (e) {
        e.preventDefault(); // prevent default link behavior
        const row = this.closest('tr'); // find the closest tr
        if (row) row.style.display = 'none'; // hide the row
    });
});
...
$(document).ready(function () {
    ...
    $("#attributeSelect").change(function () {
        ...
        if (attributeId) {
            $.ajax({
                url: '/dashboard/product-attribute/' + attributeId + '/values',
                type: 'GET',
                success: function (data) {
                    var valueSelect = $("#valueSelect");
                    valueSelect.empty();
                    valueSelect.append('<option value="">-- Select Value --</option>');
                    $.each(data, function (i, value) {
                        valueSelect.append('<option value="' + value.id + '">' + value.code + '</option>');
                    });
                }
            });
        } else {
    ...
    $("#CountrySelect").change(function () {
        ...
        if (CountryId) {
            ...
            $("#CountrySelect").empty().append('<option value="">-- Select State --</option>');
        }
    });
    //state
    $("#StateSelect").change(function () {
        ...
        if (StateId) {
            $.ajax({
                url: '/api/countries/state/' + StateId + '/cities',
                type: 'GET',
                success: function (data) {
                    var CitySelect = $("#CitySelect");
                    CitySelect.empty();
